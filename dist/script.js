...
const addInput = document.querySelector('#input'),
      ...
      textItem = document.querySelector('.todo-item__text'); // Создаём новый пустой массив для заметок. Если массив уже лежит в localStorage, то берём его оттуда
  ...
  function correctItemEvent(list) {
    list.addEventListener('click', e => {
      ...
      if (target.id === 'correct-btn') {
        ...
        target.style.display = 'none'; // Создаём форму с инпутом и кнопкой и вставляем её в item
        ...
        const correctBtn = item.querySelector('.todo-correct-form__btn'); // Ставим фокус на инпуте и устанавливаем курсор в конце

        correctInput.focus();
        correctInput.selectionStart = correctInput.selectionEnd = correctInput.value.length;
        correctBtn.addEventListener('click', e => {
          e.preventDefault(); // Проверка на пустое значение
